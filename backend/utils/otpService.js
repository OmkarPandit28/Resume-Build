      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#FFFFFF;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.10);">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#1A3A5C 0%,#185FA5 100%);padding:32px 40px;text-align:center;">
            ...
            <p style="color:#B5D4F4;font-size:13px;margin:8px 0 0;font-weight:400;">Resume Builder · Email Verification</p>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:40px;">
            <p style="font-size:16px;color:#1A1A18;margin:0 0 8px;font-weight:600;">Hi ${name} 👋</p>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:32px;">
              <tr><td align="center">
                <div style="display:inline-block;background:#F0F6FF;border:2px dashed #185FA5;border-radius:16px;padding:24px 48px;text-align:center;">
                  <p style="font-size:11px;color:#888;text-transform:uppercase;letter-spacing:2px;margin:0 0 12px;font-weight:600;">Your One-Time Password</p>
                  <p style="font-size:44px;font-weight:800;color:#0C447C;letter-spacing:12px;margin:0;font-family:'Courier New',monospace;">${otp}</p>
                  <p style="font-size:12px;color:#999;margin:12px 0 0;">⏱ Valid for <strong>${expiryMin} minutes</strong></p>
                </div>
              </td></tr>
            </table>
            ...
            <div style="background:#FFFBF0;border:1px solid #FAC765;border-radius:10px;padding:14px 18px;margin-bottom:24px;">
              <p style="font-size:13px;color:#7B4F0A;margin:0;">
                🔒 <strong>Do not share this OTP with anyone.</strong> InternHub will never ask for your OTP via call or chat.
              </p>
            </div>
...
async function createAndSendOTP(email, name = "Student") {
  ...
  await OTP.deleteMany({ email: email.toLowerCase(), purpose: "resume_payment" });

  // Save new OTP
  await OTP.create({ email: email.toLowerCase(), otp, purpose: "resume_payment", expiresAt });
...
async function verifyOTP(email, otpInput) {
  ...
  if (record.otp !== otpInput.toString().trim()) {
    ...
    return { valid: false, reason: `Incorrect OTP. ${left} attempt(s) remaining.` };
  }
...
module.exports = { createAndSendOTP, verifyOTP, isOTPVerified };
